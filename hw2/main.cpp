    ...
    while (std::getline(inputFile, line)) {
        ...
        if (cmd.command == 1 && cmd.subCommand == 1) {
            ...
            if (cmd.parameters.size() >= 3) {
                signUp.signUp(cmd.parameters[0], cmd.parameters[1], cmd.parameters[2]);
            }
        }
        else if (cmd.command == 2 && cmd.subCommand == 1) {
            // 로그인
            if (cmd.parameters.size() >= 2) {
                currentUser = login.login(cmd.parameters[0], cmd.parameters[1]);
            }
        }
        else if (cmd.command == 2 && cmd.subCommand == 2) {
            // 로그아웃
            if (currentUser) {
                logout.logout(currentUser->getId());
                currentUser = nullptr;
            }
        }
            ...
            if (currentUser && currentUser->getIsAdmin() && cmd.parameters.size() >= 2) {
                registerBike.registerBike(cmd.parameters[0], cmd.parameters[1]);
            }
        }
        else if (cmd.command == 4 && cmd.subCommand == 1) {
            // 자전거 대여
            if (currentUser && cmd.parameters.size() >= 1) {
                rentBike.rentBike(cmd.parameters[0], currentUser);
            }
        }
